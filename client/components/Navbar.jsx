...
const Navbar = () => {
  ...
  const links = [
    ...
    { to: '/library', label: 'Library', show: Boolean(user) },
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-800/80 bg-slate-950/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-3 py-2 md:px-6">
        <Link to="/" className="text-base font-bold tracking-tight text-white md:text-lg">InkScroll</Link>
        <nav className="hidden items-center gap-4 text-sm text-slate-300 md:flex">{links.map((link) => <NavLink key={link.to} to={link.to}>{link.label}</NavLink>)}</nav>
        <button className="rounded-lg p-2 text-slate-200 md:hidden" onClick={() => setOpen((v) => !v)}>{open ? <X size={18} /> : <Menu size={18} />}</button>
        ...
        <div className="hidden md:block">
          {user ? (
            <motion.div whileHover={{ y: -1 }} className="flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1.5 shadow-soft">
              ...
              <CreatorBadge creatorBadge={user.creatorBadge} />
      ...
      {open && (
        <div className="border-t border-slate-800 bg-slate-950 px-4 py-3 md:hidden">
          <div className="mb-3 flex flex-col gap-3 text-sm text-slate-300">{links.map((link) => <NavLink key={link.to} to={link.to} onClick={() => setOpen(false)}>{link.label}</NavLink>)}</div>
          {user ? (
          ...
          ) : <Link to="/auth" onClick={() => setOpen(false)} className="inline-block rounded-lg bg-white/10 px-3 py-1.5 text-sm hover:bg-white/20">Login</Link>}
        </div>
      )}
    </header>
  );
